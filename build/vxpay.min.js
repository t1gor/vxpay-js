...
var _VXPayHelperHooksConfig = __webpack_require__(385);
...
var _VXPayIsLoggedInResponseMessage = __webpack_require__(415);
		...
		value: function isLoggedIn() {
			...
			return new Promise(function (resolve, reject) {
				_this6._initPaymentFrame().then(function (vxpay) {
					try {
						// register listener
						vxpay.hooks.onIsLoggedIn(function (msg) {
							resolve(msg);
						});
...
var VXPayInitPaymentMiddleware = function VXPayInitPaymentMiddleware(vxpay, resolve) {
	...
	if (!vxpay.paymentFrame.loaded) {
		// set resolve hook
		vxpay.paymentFrame.hooks.onFlowChange(vxpay.config.updateFlow.bind(vxpay.config)).onViewReady(vxpay.paymentFrame.setVisible.bind(vxpay.paymentFrame)).onSuccess(function (msg) {
			return vxpay.paymentFrame.hide();
		}).onClose(function (msg) {
			return vxpay.paymentFrame.hide();
		}).onContentLoaded(function (msg) {
			return resolve(vxpay);
		});
